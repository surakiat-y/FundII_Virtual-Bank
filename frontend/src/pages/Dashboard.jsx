...
const Dashboard = () => {
    ...
    // 🔥 ฟังก์ชันเซฟสลิป (Nuclear Save - สร้าง Element ใหม่เพื่อเลี่ยง oklch)
    const handleSaveSlip = () => {
        ...
        clonedSlip.style.width = '350px';
        clonedSlip.style.display = 'block';
        tempContainer.appendChild(clonedSlip);

        html2canvas(clonedSlip, {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
            ...
            <main className="max-w-5xl mx-auto p-8">
                ...
                <div className="flex justify-between items-end mb-6 px-2 text-slate-800">
                    <h2 className="text-2xl font-black">My Pockets</h2>
            ...
            {/* ─── Modal Slip (Digital Receipt - Hardcoded Colors) ─── */}
            {showSlipModal && slipData && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
                    <div className="w-full max-w-sm">
                        {/* 📸 ส่วนที่จะแคป: บังคับสีขาวแบบ HEX และไม่ใช้ Class Tailwind */}
                        <div id="digital-slip" style={{ backgroundColor: '#ffffff', borderRadius: '40px', overflow: 'hidden', display: 'block', width: '100%', fontFamily: 'Arial, sans-serif' }}>
                            <div style={{ backgroundColor: '#10b981', padding: '40px 20px', textAlign: 'center' }}>
                                <div style={{ width: '60px', height: '60px', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 15px', border: '4px solid rgba(255,255,255,0.3)' }}>
                                    <span style={{ fontSize: '28px', fontWeight: 'bold', color: '#ffffff' }}>✓</span>
                                </div>
                                <h3 style={{ margin: 0, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', fontSize: '18px', color: '#ffffff' }}>Transfer Success</h3>
                                <p style={{ margin: '5px 0 0', fontSize: '10px', color: '#ffffff', opacity: 0.8 }}>{slipData.date}</p>
                            </div>

                            <div style={{ padding: '30px', backgroundColor: '#ffffff' }}>
                                <div style={{ textAlign: 'center', paddingBottom: '20px', borderBottom: '1px solid #f1f5f9', marginBottom: '20px' }}>
                                    <p style={{ margin: 0, color: '#94a3b8', fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase' }}>Amount</p>
                                    <p style={{ margin: '5px 0 0', fontSize: '36px', fontWeight: '900', color: '#0f172a' }}>฿{Number(slipData.amount).toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                                </div>
                                <div style={{ textAlign: 'left' }}>
                                    <div style={{ marginBottom: '15px' }}>
                                        <p style={{ margin: 0, color: '#94a3b8', fontSize: '9px', fontWeight: 'bold', textTransform: 'uppercase' }}>From</p>
                                        <p style={{ margin: 0, fontSize: '14px', fontWeight: 'bold', color: '#1e293b' }}>{slipData.fromName || (user.firstName + " " + user.lastName)}</p>
                                    </div>
                                    <div style={{ marginBottom: '15px' }}>
                                        <p style={{ margin: 0, color: '#94a3b8', fontSize: '9px', fontWeight: 'bold', textTransform: 'uppercase' }}>To</p>
                                        <p style={{ margin: 0, fontSize: '14px', fontWeight: 'bold', color: '#1e293b' }}>{slipData.toName}</p>
                                    </div>
                                </div>
                                <div style={{ marginTop: '20px', paddingTop: '15px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <span style={{ fontSize: '8px', color: '#cbd5e1' }}>Ref: #VB-{slipData.transactionId || '000'}</span>
                                    <span style={{ fontSize: '9px', color: '#10b981', fontWeight: 'bold', letterSpacing: '1px' }}>VIRTUAL BANK</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-6 space-y-3">
                            <button onClick={handleSaveSlip} className="w-full py-4 bg-emerald-500 text-white rounded-2xl font-black shadow-lg hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 active:scale-95">💾 Save to Gallery</button>
                            <div className="flex gap-2">
                                <button onClick={handleAddFavorite} className="flex-1 py-4 bg-white border-2 border-slate-100 text-slate-400 rounded-2xl font-black text-xs uppercase hover:bg-slate-50 transition-all tracking-widest">+ Favorite</button>
